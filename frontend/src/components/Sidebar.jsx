...
export default function Sidebar({ collapsed, setCollapsed }) {
  ...
  const width = collapsed ? '70px' : '260px';

  return (
    <>
      <aside style={{
        width, minHeight: '100vh', background: '#111',
          ...
          {navItems.map(({ to, icon: Icon, label }) => {
            ...
            return (
              <Link key={to} to={to} title={collapsed ? label : ''} style={{
                ...
                transition: 'all 0.15s', minWidth: '260px'
              }}>
        ...
        <div style={{ borderTop: '1px solid #1f1f1f', padding: '16px 20px', minWidth: '260px' }}>
          <div
            ...
            {noLeidas > 0 && (
              <span style={{ marginLeft: collapsed ? '0' : 'auto', background: '#cc0000', borderRadius: '50%', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', color: '#fff', fontWeight: 700, flexShrink: 0 }}>
                {noLeidas}
              </span>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '14px', fontWeight: 600, flexShrink: 0 }}>
              {user?.nombre?.charAt(0) || 'U'}
            </div>
            {!collapsed && (
              <>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: '#ffffff' }}>{user?.nombre || 'Usuario'}</div>
                  <div style={{ fontSize: '11px', color: '#666' }}>{user?.ciudad || ''}</div>
                </div>
                <LogOut size={16} onClick={handleLogout} style={{ marginLeft: 'auto', color: '#666', cursor: 'pointer' }} />
              </>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', top: 0, right: 0, width: '380px', height: '100vh', background: '#111', borderLeft: '1px solid #2a2a2a', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid #2a2a2a' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              <span onClick={marcarLeidas} style={{ color: '#888', fontSize: '12px', cursor: 'pointer' }}>Marcar leídas</span>
              ...
              notificaciones.map((n, i) => (
                <div key={i} style={{ padding: '16px 24px', borderBottom: '1px solid #1f1f1f', background: n.leida ? 'transparent' : 'rgba(204,0,0,0.05)', position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                    <span style={{ fontSize: '20px' }}>{iconoTipo[n.tipo] || '🔔'}</span>
                    <div>
