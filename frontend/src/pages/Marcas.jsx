...
export default function Marcas() {
  ...
  const filtradas = catActiva === 'Todas'
    ? marcas
    : marcas.filter(m => m.categoria === catActiva);

  return (
    <div>
      <h1 style={{ fontSize: '48px', fontWeight: 900, marginBottom: '24px' }}>MARCAS</h1>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
          {filtradas.map((m, i) => (
            <div key={i} style={{ background: '#1a1a1a', borderRadius: '8px', padding: '20px', border: '1px solid #2a2a2a', display: 'flex', gap: '16px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: '#666', fontSize: '11px', letterSpacing: '1px', marginBottom: '8px' }}>{m.categoria?.toUpperCase()}</div>
