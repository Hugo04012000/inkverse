...
const colorEstilo = {
  'Blackwork': '#1a1a1a', 'Japonés': '#1a0a0a', 'Watercolor': '#0a1a2a',
  'Geometric': '#0a0a1a', 'Mandala': '#1a0a1a', 'Realismo': '#0f0f0f',
  'Fineline': '#0a1a0a', 'Neo-tradicional': '#1a0800', 'Tradicional': '#1a1000',
  'Tribal': '#111111', 'Lettering': '#0a0a12', 'Anime': '#12001a',
  'Biomecánico': '#0a1212', 'Cover-up': '#121200', 'Minimalista': '#101010',
  'Puntillismo': '#0a120a', 'Surrealista': '#12000a', 'Floral': '#0a1200', 'Celtíco': '#001212',
};
...
export default function Disenos() {
  ...
  const [form, setForm] = useState({ titulo: '', estilo: 'Geometric' });
  ...
  function subirDiseno() {
    ...
    setSubiendo(true);
    if (imagenBase64) {
      api.post('/upload/imagen', { imagen: imagenBase64 }).then(function(res) {
        return api.post('/disenos', { titulo: form.titulo, estilo: form.estilo, imagen_url: res.data.url });
      }).then(function() {
        setNuevoDiseno(false);
        setForm({ titulo: '', estilo: 'Geometric' });
      ...
      api.post('/disenos', { titulo: form.titulo, estilo: form.estilo }).then(function() {
        setNuevoDiseno(false);
        setForm({ titulo: '', estilo: 'Geometric' });
        setSubiendo(false);
        cargarDisenos();
        ...
        <div onClick={function() { setNuevoDiseno(false); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div onClick={function(e) { e.stopPropagation(); }} style={{ background: '#1a1a1a', borderRadius: '8px', padding: '32px', width: '100%', maxWidth: '500px', border: '1px solid #333' }}>
            ...
            <input
              ...
              value={form.titulo}
              onChange={function(e) { setForm({ titulo: e.target.value, estilo: form.estilo }); }}
            ...
            <select
              value={form.estilo}
              onChange={function(e) { setForm({ titulo: form.titulo, estilo: e.target.value }); }}
              style={{ width: '100%', background: '#111', border: '1px solid #333', borderRadius: '4px', padding: '12px', color: '#ffffff', marginBottom: '12px', fontSize: '14px', boxSizing: 'border-box' }}>
            ...
            <div
              ...
              style={{ width: '100%', height: '160px', background: '#111', border: '2px dashed #333', borderRadius: '4px', marginBottom: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', boxSizing: 'border-box' }}>
              {imagenPreview ? (
                <img src={imagenPreview} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <>
                  <span style={{ fontSize: '32px', marginBottom: '8px' }}>📷</span>
                  <span style={{ color: '#666', fontSize: '13px' }}>Haz clic para subir una imagen</span>
                  <span style={{ color: '#444', fontSize: '11px', marginTop: '4px' }}>JPG, PNG o WebP</span>
                </>
              )}
            </div>
            <input ref={inputRef} type="file" accept="image/*" onChange={seleccionarImagen} style={{ display: 'none' }} />

            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={subirDiseno} disabled={subiendo} style={{ flex: 1, background: '#cc0000', color: '#ffffff', fontWeight: 700, padding: '12px', borderRadius: '4px', border: 'none', cursor: 'pointer', opacity: subiendo ? 0.7 : 1 }}>
                {subiendo ? 'SUBIENDO...' : 'PUBLICAR'}
              </button>
              <button onClick={function() { setNuevoDiseno(false); setImagenPreview(null); setImagenBase64(null); }} style={{ flex: 1, background: '#333', color: '#ffffff', padding: '12px', borderRadius: '4px', border: 'none', cursor: 'pointer' }}>
                CANCELAR
              </button>
            </div>
          </div>
